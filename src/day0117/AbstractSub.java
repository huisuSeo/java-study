...
/**
 * 추상클래스의 자식 클래스.<br>
 * 부모클래스의 추상 method를 반드시 Override해야한다.<br>
...
//		new AbstractSuper();//추상클래스는 직접 객체화 할 수 없다.
		...
		//상속관계의 객체화.(자식은 부모 이다. => is a 관계)
		//자식클래스에서는 부모클래스의 정보를 알 수 있기 떄문에 자식클래스의 객체는 부모클래스로
		//만들어진 객체에 할당 될 수 있다.
		System.out.println("------------------------------------------------");
//		AbstractSub as2 = new AbstractSuper();//부모는 자식을 모름.
		AbstractSuper as2 = new AbstractSub();//is a 관계의 객체화
		//객체는 부모의 정보를 사용하기 때문에 부모의 기능만 사용할 수 있다.
		as2.methodA();//자신의 method
		as2.methodB();//자신의 method는 body가 없어 일을 할 수 없지만, 
		//주소를 할당받은 자식의 method가 최우선적으로 호출된다.
		System.out.println(as2.methodC("창밖에 눈 온다"));
