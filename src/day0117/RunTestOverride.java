...
public class RunTestOverride {
	...
	public String toString() {
		return "객체가 출력될 때 주소가 아닌 다른 메시지 출력하도록 설정 " + super.toString();
	}//toString
...
//		tos.sub();//자식클래스가 가진 자원은 사용할 수 없다.
		...
		System.out.println("----------------------------------------------------------");
		//자식클래스로 객체화 : 부모클래스의 모든 자원과 자식 클래스의 모든 자원을 사용할 수 있다.
		...
		System.out.println("----------------------------------------------------------");
		RunTestOverride rto/* heap의 주소 저장*/ = new RunTestOverride();
		System.out.println(rto.toString());//heap주소
		
		String str = new String("오늘은 월요일 입니다.");
		System.out.println(str);//메시지 출력
